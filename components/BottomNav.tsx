...
const BottomNav: React.FC = () => {
  ...
  const activeLink = 'text-indigo-600 scale-110';
  const inactiveLink = 'text-slate-500';

  return (
    <nav className="flex-shrink-0 w-full h-16 bg-white/80 backdrop-blur-sm border-t border-slate-200 mx-auto max-w-md">
      <div className="flex justify-around items-center h-full">
        {navItems.map(({ path, label, Icon }) => (
          <NavLink
            ...
            aria-label={label}
            className={({ isActive }) => 
              `flex items-center justify-center transition-all duration-200 transform ${isActive ? activeLink : inactiveLink} hover:text-indigo-500`
            }
          >
