...
const ProfessionalChatView: React.FC<ProfessionalChatViewProps> = ({ professional, onClose }) => {
    ...
    return (
        <motion.div
            {...motionProps}
            className="h-full flex flex-col absolute top-0 left-0 right-0 bottom-0 bg-white"
        >
            <header className="flex items-center p-4 border-b border-slate-200 flex-shrink-0">
                ...
                <button 
                    ...
                    className="flex flex-col items-center flex-grow -ml-4 hover:bg-slate-200 rounded-lg p-1 transition-colors focus:outline-none focus:ring-2 focus:ring-slate-300"
                    aria-label={`Ver detalhes de ${professional.name}`}
                >
                    <h1 className="text-xl font-bold text-slate-800">{professional.name}</h1>
                    <p className="text-sm text-blue-500">{professional.specialty}</p>
                </button>
            </header>
