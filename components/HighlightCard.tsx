...
const HighlightCard: React.FC<HighlightCardProps> = ({ highlight, isAddNew = false, onClick }) => {
    if (isAddNew) {
        return (
             <button
                onClick={onClick}
                className="w-24 h-36 flex-shrink-0 flex flex-col items-center justify-center space-y-2 rounded-xl border-2 border-dashed border-slate-300 bg-slate-100/70 hover:bg-slate-200/70 transition-colors text-slate-500"
            >
    ...
    return (
        <button onClick={onClick} className="w-24 h-36 flex-shrink-0 relative rounded-xl overflow-hidden group shadow-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
            <img src={highlight.coverUrl} alt={highlight.label} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" />
