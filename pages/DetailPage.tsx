    ...
    if (!('title' in item) || !('content' in item)) {
        return (
            <motion.div {...motionProps} className="p-6 text-center h-full flex flex-col justify-center items-center">
                <h2 className="text-xl font-bold">Conteúdo Inválido</h2>
                <p className="text-slate-500 mt-2">O item que você está tentando ver não pode ser exibido aqui.</p>
    ...
    const renderRecipeContent = (content: string) => {
        return content.split('\n').map((line, i) => {
            if (line.startsWith('**') && line.endsWith('**')) {
                return <h2 key={i} className="text-xl font-bold text-slate-800 mt-6 mb-2">{line.replace(/\*\*/g, '')}</h2>
            }
            if (line.trim() === '') return null; // Não renderiza parágrafos vazios de linhas em branco
            return <p key={i} className="text-slate-600 my-2 leading-relaxed">{line}</p>
        });
    };
            ...
            <main className="flex-grow overflow-y-auto pb-6">
                 {isWorkoutWithSteps ? (
                    <div className="p-6 space-y-6">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-slate-800 mb-1">{item.title}</h1>
                            {'description' in item && <p className="text-slate-500 mb-6">{item.description}</p>}
                            {'steps' in item && item.steps && (
                                <div className="space-y-3">
                                    {item.steps.map((step, index) => {
                                        const Icon = step.type === 'exercise' ? DumbbellIcon : ClockIcon;
                                        return (
                                            <div key={index} className="flex items-center space-x-4 p-4 bg-slate-100 rounded-2xl">
                    ...
                    <>
                        ...
                        <div className="p-6 space-y-4">
                             ...
                             <h1 className="text-3xl font-bold text-slate-800 flex items-center gap-3">
                                {isRecipe && 'Icon' in item && <item.Icon className="h-8 w-8 text-slate-700" />}
                                <span>{item.title}</span>
                            </h1>
                            {isRecipe ? (
                                <div>{renderRecipeContent(item.content)}</div>
                            ) : (
                                <p className="text-base whitespace-pre-line text-slate-600 leading-relaxed">
                                    {item.content}
                                </p>
                            )}
                        </div>
