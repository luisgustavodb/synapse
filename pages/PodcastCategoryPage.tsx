    ...
    if (!category) {
        return (
            <motion.div {...motionProps} className="h-full flex flex-col items-center justify-center">
                <p>Categoria de podcast não encontrada.</p>
                <button onClick={goBack} className="mt-4 text-indigo-600 font-semibold">Voltar</button>
            </motion.div>
        );
    }
        ...
        <motion.div {...motionProps} className="h-full flex flex-col bg-slate-50">
             <header className="flex-shrink-0 bg-white/80 backdrop-blur-sm sticky top-0 z-20">
                <div className="flex items-center p-4 border-b border-slate-200">
                    ...
                    <h1 className="text-2xl font-bold text-slate-800 mx-auto pr-8">{category.title}</h1>
                </div>
            </header>
            <main className="flex-grow overflow-y-auto space-y-8 py-6">
                {popularEpisodes.length > 0 && (
                    <section>
                        <h2 className="text-xl font-bold text-slate-800 px-4 sm:px-6 mb-3">Populares em {category.title}</h2>
                ...
                {recentEpisodes.length > 0 && (
                     <section>
                        <h2 className="text-xl font-bold text-slate-800 px-4 sm:px-6 mb-3">Lançamentos Recentes</h2>
                         <div className="overflow-x-auto no-scrollbar">
                            <div className="flex space-x-4 px-4 sm:px-6">
