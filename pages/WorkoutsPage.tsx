...
const ExerciseListItem: React.FC<{ item: ActivityItem }> = ({ item }) => (
    <Link to={`/detail/activity/${item.id}`} className="block transition-colors hover:bg-slate-100/70 active:bg-slate-200/60">
        <div className="flex items-center space-x-4 p-3 sm:p-4">
            ...
            <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-slate-500 truncate">{item.muscleGroup}</p>
            </div>
            <TrendingUpIcon className="w-6 h-6 text-slate-400 flex-shrink-0" />
        </div>
    </Link>
);
...
const WorkoutsPage: React.FC = () => {
    ...
    return (
        <motion.div {...motionProps} className="h-full flex flex-col bg-slate-50">
            <header className="flex-shrink-0 bg-white/80 backdrop-blur-sm sticky top-0 z-20">
                <div className="flex items-center p-4 border-b border-slate-200">
                    ...
                    <h1 className="text-xl font-bold text-slate-800 mx-auto pr-8">Explore os Treinos</h1>
                </div>
            </header>

            <div className="p-4 space-y-4 flex-shrink-0 bg-white border-b border-slate-200 sticky top-[65px] z-10">
                <div className="relative">
                    ...
                    <input
                        ...
                        className="w-full bg-slate-100 border border-slate-200 rounded-lg py-2.5 pl-10 pr-4 focus:outline-none focus:ring-2 focus:ring-indigo-400 text-slate-800 text-sm"
            ...
            <main className="flex-grow overflow-y-auto">
                 <h2 className="text-lg font-bold text-slate-800 px-4 pt-4 pb-2">Treinos Populares</h2>
                <AnimatePresence>
                     {filteredWorkouts.length > 0 ? (
                        ...
                        >
                         ...
                         >
                            <div className="inline-block p-4 bg-slate-200 rounded-full">
                                <LeafIcon className="h-10 w-10 text-slate-400" />
                            </div>
                            <h3 className="mt-4 text-lg font-bold text-slate-700">Nenhum exercício encontrado</h3>
                            <p className="text-sm text-slate-500">Tente uma busca ou filtros diferentes para ver mais resultados.</p>
                         </motion.div>
                     )}
                </AnimatePresence>
            </main>
