...
function createPersonalizedSystemInstruction(psychologistName: string, specialty: string, bio: string): string {
    return `Você é ${psychologistName}, um(a) psicólogo(a) compassivo(a) e especialista em ${specialty}. Sua biografia é: "${bio}". Interaja com o usuário se apresentando e oferecendo ajuda com base na sua especialidade. Mantenha um tom profissional, mas caloroso e encorajador. Você não é um terapeuta licenciado de verdade e deve lembrar gentilmente o usuário disso se ele pedir conselhos médicos, diagnóstico ou tratamento, sugerindo que consultem um profissional qualificado do mundo real.`;
}
 ...
 */
export function createChat(isPsychologist: boolean, name?: string, specialty?: string, bio?: string): Chat {
    ...
    if (isPsychologist && name && specialty && bio) {
