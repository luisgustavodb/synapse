...
const ChatConversationItem: React.FC<{ conversation: ChatConversation }> = ({ conversation }) => {
    ...
    return (
        <div className="flex items-center p-3 space-x-4 hover:bg-slate-100 rounded-lg transition-colors cursor-pointer mx-2">
            <div className="relative flex-shrink-0">
                <div className={`w-14 h-14 rounded-full flex items-center justify-center text-2xl overflow-hidden ${!isAvatarUrl ? 'bg-slate-200' : ''}`}>
                    {isAvatarUrl ? (
            ...
            <div className="flex-grow overflow-hidden">
                <p className="font-semibold text-slate-800 truncate">{conversation.user.name}</p>
                <div className="flex items-center text-sm">
                    <p className={`flex-grow truncate ${conversation.isRead ? 'text-slate-500' : 'text-slate-800 font-medium'}`}>{conversation.lastMessage}</p>
                    <span className="text-slate-500 flex-shrink-0 ml-2">· {conversation.timestamp}</span>
                </div>
            </div>
            {!conversation.isRead && (
    ...
    return (
        <AnimatePresence>
            {isOpen && (
                ...
                >
                    <header className="flex items-center p-4 border-b border-slate-200 flex-shrink-0 bg-slate-50/80 backdrop-blur-sm sticky top-0">
                        ...
                        <h1 className="text-lg font-bold text-slate-800 flex-1 text-center">Mensagens</h1>
                        <div className="flex-1"></div>
                    </header>
                    <div className="p-4 flex-shrink-0">
                        <div className="relative">
                            <input type="search" placeholder="Pesquisar" className="w-full bg-white border border-slate-300 rounded-lg py-2.5 pl-10 pr-4 focus:outline-none focus:ring-2 focus:ring-indigo-400 text-slate-800 text-sm" />
